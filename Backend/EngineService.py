...
from Backend.VoiceOutput import VoiceOutput, installed_voices, startup_greeting
...
class EngineService:
    def __init__(self, config, emit, *, start_workers=True, services=None, voice_factory=VoiceInput, speaker_factory=VoiceOutput):
        ...
        self._personal_cancel = threading.Event()
    ...
    def snapshot(self):
        ...
        from Backend.OwnerEnrollment import status as owner_status
        return {
            ...
            'owner': owner_status(self.config),
            'personal_voice': self.speaker.voice_status() if hasattr(self.speaker, 'voice_status') else {},
    ...
    def handle(self, request):
        ...
        if op == 'connect':
            ...
            if not self._connected:
                ...
                if self.config.settings.voice_startup and not request.get('no_listen', False):
                    self.speaker.say(startup_greeting(self.config.settings))
        ...
        if op == 'personal_voice_status':
            return self.speaker.voice_status()
        if op == 'personal_voice_reload':
            return self.speaker.reload()
        if op in {'personal_voice_record', 'personal_voice_import', 'personal_voice_delete'}:
            from Backend.PersonalVoice import record_reference, import_reference, delete_reference
            if not self._personal_lock.acquire(blocking=False):
                raise ValueError('A personal voice operation is already running.')
            try:
                self._personal_cancel.clear()
                self.speaker.stop()
                if op == 'personal_voice_record':
                    self.voice.set_enabled(False)
                    self.emit('personal_voice_progress', 'Recording for 8 seconds. Speak naturally. Stop cancels recording.')
                    record_reference(self.config, self._personal_cancel, consent=request['consent'],
                        progress=lambda message: self.emit('personal_voice_progress', message))
                elif op == 'personal_voice_import':
                    import_reference(self.config, request['path'], consent=request['consent'])
                else:
                    delete_reference(self.config)
                return self.speaker.voice_status()
            finally:
                self.emit('personal_voice_progress', '')
                self._personal_lock.release()
        if op == 'owner_status':
            from Backend.OwnerEnrollment import status
            return status(self.config)
        if op == 'owner_set_pin':
            from Backend.OwnerEnrollment import set_pin, status
            set_pin(self.config, request['pin'])
            return status(self.config)
        if op == 'owner_delete_pin':
            from Backend.OwnerEnrollment import delete_pin, status
            delete_pin(self.config)
            return status(self.config)
        if op == 'owner_voice_enroll':
            from Backend.OwnerEnrollment import record_voice_samples, status
            progress = lambda message: self.emit('owner_progress', {'message': message})
            record_voice_samples(self.config, samples=request.get('samples', 3), seconds=request.get('seconds', 4), device=request.get('device'), progress=progress)
            return status(self.config)
        if op == 'owner_delete_voice':
            from Backend.OwnerEnrollment import delete_voice, status
            delete_voice(self.config)
            return status(self.config)
        if op == 'owner_face_camera':
            from Backend.OwnerEnrollment import enroll_face_camera, status
            progress = lambda message: self.emit('owner_progress', {'message': message})
            enroll_face_camera(self.config, samples=request.get('samples', 8), camera=request.get('camera'), progress=progress)
            return status(self.config)
        if op == 'owner_face_images':
            from Backend.OwnerEnrollment import enroll_face_images, status
            progress = lambda message: self.emit('owner_progress', {'message': message})
            enroll_face_images(self.config, request['paths'], progress=progress)
            return status(self.config)
        if op == 'owner_delete_face':
            from Backend.OwnerEnrollment import delete_face, status
            delete_face(self.config)
            return status(self.config)
        if op == 'files':
        ...
        if op == 'voice_test':
            name = self.config.settings.assistant_name
            self.speaker.say(f'Hello. I am {name}. Your desktop is at your command.', force=True)
            return {'started': True}
...
REQUEST_FIELDS = {
    ...
    'owner_status': {}, 'owner_set_pin': {'pin': str}, 'owner_delete_pin': {},
    'owner_voice_enroll': {'samples': int, 'seconds': int, 'device': int},
    'owner_delete_voice': {}, 'owner_face_camera': {'samples': int, 'camera': int},
    'owner_face_images': {'paths': list}, 'owner_delete_face': {},
    'personal_voice_status': {}, 'personal_voice_reload': {}, 'personal_voice_delete': {},
    'personal_voice_record': {'consent': bool}, 'personal_voice_import': {'path': str, 'consent': bool},
}
REQUIRED_FIELDS = {'command': {'text'}, 'listen': {'enabled'}, 'settings': {'values'},
                   'autostart_authorize': {'enabled'}, 'autostart_result': {'ticket', 'success'},
                   'owner_set_pin': {'pin'}, 'owner_face_images': {'paths'}}
REQUIRED_FIELDS.update({'personal_voice_record': {'consent'}, 'personal_voice_import': {'path', 'consent'}})
...
def validate_request(request):
    ...
    if request['op'] == 'owner_face_images' and (not request['paths'] or len(request['paths']) > 12 or not all(isinstance(p, str) and p for p in request['paths'])):
        raise ValueError('Provide one to twelve local image paths.')
    if request['op'] == 'owner_set_pin' and not 4 <= len(request['pin']) <= 128:
        raise ValueError('PIN/password must contain 4 to 128 characters.')
